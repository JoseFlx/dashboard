...
with st.sidebar.expander("Configurações avançadas", expanded=False):
    ...
    tma_warn_min = st.number_input("TMA aviso — amarelo (min)", min_value=0, value=5)
    tma_crit_min = st.number_input("TMA crítico — vermelho (min)", min_value=0, value=10)
    tma_warn_s = int(tma_warn_min * 60)
    tma_crit_s = int(tma_crit_min * 60)
    st.markdown("---")
    sla_target = st.number_input("SLA alvo (segundos)", min_value=1, value=20)
    ...
    sla_thresholds = st.multiselect("Alvos de SLA (s) - curva", options=[5,10,20,30,60], default=[5,10,20])
